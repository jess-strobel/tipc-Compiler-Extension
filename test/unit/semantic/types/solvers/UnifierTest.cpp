...
TEST_CASE("Unifier: Collect and then unify constraints", "[Unifier, Collect]") {
  ...
  SECTION("Test type-safe program 2") {
    std::stringstream program;
    program << R"(
    ...
    //auto zInferred = unifier.inferred(std::make_shared<TipVar>(symbols->getLocal("z", fDecl)));
    // auto zFun = std::dynamic_pointer_cast<TipArray>(zInferred);

    // Expected types
    std::vector<std::shared_ptr<TipType>> emptyParams;
    std::shared_ptr<TipType> alphaParam = std::make_shared<TipAlpha>(symbols->getLocal("z", fDecl));
  ...
  SECTION("Test type-safe program 3") {
    std::stringstream program;
    program << R"(
            // x is bool, y is bool, z is bool, short is () -> bool]
            short() {
              var x, y, z;
              x = true;	
              y = (x != true);
    ...
    // Expected types
    auto boolType = std::make_shared<TipBool>();
    auto funRetInt = std::make_shared<TipFunction>(emptyParams, std::make_shared<TipBool>());
  ...
  SECTION("Test type-safe program 4") {
    std::stringstream program;
    program << R"(
            // x is bool, y is bool, z is bool, short is () -> int]
            short() {
              var x, y, z;
              x = (5 <= 3);
              y = (2 < 1) and ((5 % 3) == 2);
...
deref(p){
  ...
  SECTION("Test unification error 6") {
    std::stringstream program;
    program << R"(
        foo() {
            var r, q;
  ...
  SECTION("Test unification error 7") {
    std::stringstream program;
    program << R"(
        foo() {
            var r, q;
  ...
  SECTION("Test unification error 8") {
    std::stringstream program;
    program << R"(
        foo() {
            var r, q;
